...
function rwDep(callback) {
  ...
  .toArray(function(err, result) {
    if (err) callback(err);
    for (var i = 0; i < result.length; i++) {
      result[i].code = result[i]._id.code;
      result[i].abbr = result[i]._id.abbr;
      result[i]._id = univ._id + "000000".concat(result[i].code).substr(-6) ;
...
function rwDiv(callback) {
  ...
  .toArray(function(err, result) {
    if (err) callback(err);
    for (var i = 0; i < result.length; i++) {
      result[i].code = result[i]._id.divCode;
      result[i].abbr = result[i]._id.divAbbr;
  ...
  .toArray(function(err, result) {
    ...
    for (var i = 0; i < result.length; i++) {
      ...
      for(var j = 0; j < result[i].contracts.length; j++) {
        ...
        mtu.estimate.remains += result[i].contracts[j].estimate.remains;
        mtu.estimate.plan += result[i].contracts[j].estimate.plan;
        mtu.estimate.income += result[i].contracts[j].estimate.income;
        mtu.estimate.outlayO += result[i].contracts[j].estimate.outlayO;
        mtu.estimate.outlay += result[i].contracts[j].estimate.outlay;
        mtu.estimate.balance += result[i].contracts[j].estimate.balance;
        mtu.estimate.balanceE += result[i].contracts[j].estimate.balanceE;
        mtu.estimate.balanceWO += result[i].contracts[j].estimate.balanceWO;
      }
