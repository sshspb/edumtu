    ...
    [ {
        $group : { 
          _id : { 
            ...
            depCode:"$depCode", divCode: "$divCode", steward: "$steward"
          },
          remains: { $sum: "$remains"},
          plan: { $sum: "$plan"},
          income: { $sum: "$income"},
...
function stewardContracts(callback) {
  db.collection('contracts').aggregate([ 
      { $group : { _id : { steward : "$steward", scope: "$scope"} } },
      { $sort: { "_id.steward": 1 } },
      { $lookup: {
          from: "contracts",
          localField: "_id",
...
function depEstimate1(callback) {
  ...
  .toArray(function(err, result) {
    ...
    const collName = 'departments_buffer';
    dropCollectionIfExist(db, collName, function (err) {
  ...
  .toArray(function(err, result) {
    ...
    for (var i = 0; i < result.length; i++) {
      if (result[i].contracts[0]) {
        ...
        result[i].source = result[i].contracts[0].source;
        } else {
        result[i].parent = '';
        result[i].steward = '';
        result[i].scope = '';
