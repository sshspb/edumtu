...
8. Создать для администратора сайта запись login/password `admin/admin` в коллекции `users`

    `node lib/initAdmin.js`

9. Запускается веб-сервер командой

    `npm start`
    
Прервать работу веб-сервера можно по `<Ctrl>+C`
 
10. В адресной строке веб-броузера набрать адрес `localhost:3000`. На появившейся странице сайта в меню есть пункт `"Импорт данных"` - жмём - данные загрузятся из указанной в файле `config_local.js` папки `"tbPath"`.

11. Чтобы обновить версию сайта запускаем командный файл
    `pull.bat`
...
При импорте исходных данных из файла `Inet_MUSmetaFin.txt` мы извлекаем реестр договоров с их сметами и заносим в коллекцию `estimates`, а в коллекцию `contracts` вспомогательный реестр договоров без приложения смет.

Из того же файла извлекаем список ФИО руководителей договоров. Этот список есть весь кадровый список, заносим его в  коллекцию `stewards` - реестр кадров.

Оттуда же, из файла `Inet_MUSmetaFin.txt`, извлекаем организационную структуру университета и заносим в коллекцию `departments` - реестр приказов об организационной структуре.

Администратор сайта может назначить руководителем подразделения человека из кадрового списка, то есть ввести в коллекцию `chiefs`, в реестр приказов о назначении руководителя подразделения, приказ о назначении определённого работника, из реестра кадров `stewards`, руководителем определённого, из реестра `departments` , подразделения.